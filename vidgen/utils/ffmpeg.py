...
def final_mix(video_path:str,output_path:str,subtitle_path:Optional[str]=None,narration_path:Optional[str]=None,music_path:Optional[str]=None,dialogue_tracks=None,**_):
    ...
    filters = [f"[0:v]{vf}[v]", "[0:a]aresample=48000,volume=0.28[amb]",
               "[1:a]aresample=48000,apad[narr]", "[2:a]aresample=48000,volume=0.20[music]"]
    speech_labels = ["narr"]
    for index, track in enumerate(dialogue_tracks or []):
        path = track.get("path")
    ...
    filters.append("".join(f"[{x}]" for x in speech_labels) + f"amix=inputs={len(speech_labels)}:duration=first:normalize=0[speech]")
    filters.append("[music][speech]sidechaincompress=threshold=0.02:ratio=8:attack=20:release=400[duck]")
    filters.append("[amb][duck][speech]amix=inputs=3:duration=first:normalize=0,loudnorm=I=-16:TP=-1.5:LRA=11[a]")
